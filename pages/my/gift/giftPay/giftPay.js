...
Page({
  ...
  onLoad: function (e) {
    ...
    wx.request({
      url: app.globalData.config.host + '/home/package_order_detail',
    ...
    wx.requestPayment({
      ...
      success: function (mes) {
        if (mes.errMsg == "requestPayment:ok") {
          app.success('支付成功', 1500, '/pages/my/gift/giftOrderList/giftOrderList?type=2')
        }
      },
      fail: function (mes) {
        if (mes.errMsg == "requestPayment:fail") {
          app.error('支付已取消', 1500, '/pages/my/gift/giftOrderList/giftOrderList?type=0')
        } else {
          app.error('支付失败！', 1500, '/pages/my/gift/giftOrderList/giftOrderList?type=0')
        }
      }
    })
  },
