 ...
 */
  ...
  getCartList: function () {
    ...
    /** 获取购物车列表 */
    wx.request({
      url: app.globalData.config.host + '/home/recipe_cart_list',
      method: 'post',
  ...
  cartUpdate: function (datas) {
    ...
    app.checkLogin()
    wx.request({
      url: app.globalData.config.host + '/home/recipe_cart_update',
      method: 'post',
      header: { "Content-Type": "application/x-www-form-urlencoded" },
      data: {
  ...
  deleteList(e) {
    ...
    app.checkLogin()
    wx.request({
      url: app.globalData.config.host + '/home/recipe_cart_remove',
      method: 'post',
      header: { "Content-Type": "application/x-www-form-urlencoded" },
      data: {
        session_key: wx.getStorageSync('session_key'),
  ...
  submitCart: function () {
    ...
    if (is_check) {
      ...
      */
      wx.navigateTo({
        url: '/pages/recipesubmitOrder/recipesubmitOrder?type=cart&id=' + cart_id
      })
    } else {
      app.error('请选择商品！');
    }
