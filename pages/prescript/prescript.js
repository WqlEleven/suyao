...
Page({
  ...
  getCartList: function () {
    ...
    /** 获取购物车列表 */
    wx.request({
      url: app.globalData.config.host + '/home/prescript_cart_list',
      method: 'post',
  ...
  cartUpdate: function (datas) {
    ...
    app.checkLogin()
    wx.request({
      url: app.globalData.config.host + '/home/prescript_cart_update',
      method: 'post',
      header: { "Content-Type": "application/x-www-form-urlencoded" },
      data: {
  ...
  deleteList(e) {
    ...
    app.checkLogin()
    wx.request({
      url: app.globalData.config.host + '/home/prescript_cart_remove',
      method: 'post',
      header: { "Content-Type": "application/x-www-form-urlencoded" },
      data: {
        session_key: wx.getStorageSync('session_key'),
  ...
  submitCart: function () {
    ...
    if (is_check) {
      ...
      */
      ...
      app.loading()
      wx.request({
        url: app.globalData.config.host + '/home/prescript',
        data: {
          session_key: session_key,
          cartId: cart_id
        },
        ...
        success: function (mes) {
          app.hide()
          if (!mes.data.errCode) {
            app.success('成功', 1500, '/pages/prescriptDetail/prescriptDetail?id=' + mes.data.order_id)
          } else {
            app.error(mes.data.msg)
          }
        }
